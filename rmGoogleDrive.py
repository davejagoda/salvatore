...
def get_file_ids(drive_service, filename, verbose):
    q = 'title="{}" and mimeType="application/vnd.google-apps.document"'.format(
        filename)
...
if '__main__' == __name__:
    ...
    for file_id in get_file_ids(drive_service, args.filename, args.verbose):
        ...
        print(utils.get_document_contents_from_drive(drive_service, file_id,
                                                     args.verbose))
        response = input('enter "y" delete file ')
        if 'y' == response:
