...
if '__main__' == __name__:
    ...
    parser.add_argument('old_folder', help='the old folder')
    parser.add_argument('new_folder', help='the new folder')
    args = parser.parse_args()
    drive_service = utils.get_drive_service(args.tokenFile, args.verbose)
    old_id = utils.get_folder_id(drive_service, args.old_folder, args.verbose)
    if None == old_id:
    ...
    if None == new_id:
        ...
        sys.exit(1)
    items = utils.get_folder_contents_from_id(drive_service, old_id, args.verbose)
    for item in items:
        if args.verbose:
