...
def list_drive(drive_service, name, pages_requested, verbose):
    pages_returned = 0
    result = []
    page_token = None
    while True:
...
def get_name_from_id(drive_service, id, verbose):
    if id not in id_to_name_hash:
        (title, type) = utils.get_document_title_and_type_from_id(
            drive_service, id, verbose)
        id_to_name_hash[id] = title
    return(id_to_name_hash[id])

def print_result(drive_service, result, md5, count_parents, name_parents,
                 verbose):
    for item in result:
        if md5:
            if 'md5Checksum' in item:
                ...
                sum = '0'*32
            print('{} {}'.format(sum, item['title']))
        if count_parents:
            print('{} {}'.format(len(item['parents']), item['title']))
        if name_parents:
            if 1 == len(item['parents']):
                print('{:32} {}'.format(get_name_from_id(
                    drive_service, item['parents'][0]['id'], verbose),
                                        item['title']))
            if 0 == len(item['parents']):
                print('{:^32} {}'.format('None', item['title']))
            if 1 < len(item['parents']):
                print('{:^32} {}'.format('Multiple', item['title']))
...
if '__main__' == __name__:
    ...
    parser.add_argument('-p', '--pages', type=int, default=0,
                        help='pages of results to show (omit to get all pages)')
    ...
    group.add_argument('-c', '--count_parents', action='store_true',
                       help='show parent counts')
    group.add_argument('-n', '--name_parents', action='store_true',
                       help='show parent names')
    ...
    if args.raw:
        ...
        print_result(drive_service, result, args.md5, args.count_parents,
                     args.name_parents, args.verbose)
